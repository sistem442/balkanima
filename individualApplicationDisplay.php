...
$sql = mysql_query("SELECT * FROM prijave ORDER BY id ASC");
...
$sql2 = mysql_query("SELECT * FROM prijave ORDER BY id ASC $limit"); 
...
while($row = mysql_fetch_array($sql2)){ 
    ...
    $id = $row["id"];
    $oriTitle = $row["oriTitle"];
    $engTitle = $row["engTitle"];

    $outputList .= '<h1>' . $oriTitle. '</h1><h2>' . $engTitle. ' </h2><hr />';
